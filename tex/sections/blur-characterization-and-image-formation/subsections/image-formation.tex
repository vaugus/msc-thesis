...
\noindent where $I_{0} = (C \pi R^{2})^{2}$ is the intensity for $\theta = 0$, $C$ is a constant, $R$ is the radius of the aperture, $\lambda$ is the wavelength of the light, $a$ is the diameter of the aperture and $J_{1}$ is the Bessel function of the first kind and first order \cite{mathews1970mathematical}. The Bessel function for the general case of $r^{th}$ order is given by
...
\begin{figure}[htb]
	\centering
	\caption{\label{fig:point_spread_function} Light rays from a point source go through a lens' surface and converge to a point in the image plane (a) and the scheme of a point spread function generated by a focused diffraction-limited system with incoherent light (b).} 
	\begin{center}
